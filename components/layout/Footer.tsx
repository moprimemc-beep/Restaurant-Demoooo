...
import { restaurant, navigation, openingHours, socialLinks } from "@/lib/data";
    ...
    <footer className="tone-dark bg-secondary text-cream">
      <Container className="py-16 sm:py-20">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr_1fr_1.1fr]">
          <div>
            ...
            <p className="mt-5 max-w-xs text-sm leading-relaxed text-cream/70">
              {restaurant.slogan} Seit {restaurant.foundingYear} authentische italienische
              Küche im Herzen von {restaurant.city}.
            </p>
                ...
                return (
                  <li key={social.label}>
                    <span
                      ...
                      aria-label={`${social.label}: ${social.handle} (Demo-Website, kein aktiver Link)`}
          ...
          <div>
            <h3 className="font-display text-base italic text-accent">Navigation</h3>
            <ul className="mt-5 space-y-3 text-sm">
              {navigation.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="text-cream/75 transition-colors hover:text-accent">
                    {item.label}
                  </Link>
                </li>
              ))}
              <li>
                <Link href="/reservierung" className="text-cream/75 transition-colors hover:text-accent">
                  Reservierung
                </Link>
              </li>
            </ul>
          ...
          <div>
            <h3 className="font-display text-base italic text-accent">Öffnungszeiten</h3>
          ...
          <div>
            <h3 className="font-display text-base italic text-accent">Kontakt</h3>
        ...
        <div className="mt-14 flex flex-col gap-5 border-t border-cream/15 pt-10 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="font-display text-base italic text-accent">Newsletter</h3>
            <p className="mt-2 max-w-sm text-sm text-cream/70">
              Neuigkeiten aus der Trattoria — von der Speisekarte bis zu besonderen Abenden.
            </p>
          </div>
          <NewsletterForm />
        </div>
        ...
        <div className="mt-10 flex flex-col gap-4 border-t border-cream/15 pt-8 text-xs text-cream/60 sm:flex-row sm:items-center sm:justify-between">
          <p>
            © {new Date().getFullYear()} {restaurant.name} · Mit Sorgfalt für das Wesentliche.
          </p>
